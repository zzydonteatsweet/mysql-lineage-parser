...
1. **静态检查过**：`mvn clean verify -DskipTests` → BUILD SUCCESS（PMD/SpotBugs/Checkstyle 对改动文件无新增告警）。
2. **目标类达标**：`SelectLineageVisitorTest` → **43 tests / 18 fail**（新增绿 6 个：3 穿透 + `directMappingTransformation` + `selfJoinWithAliases` + `bareColumnAliasShouldResolveToPhysicalColumn`）。
3. **全仓零回归**：`mvn clean test` 合计失败数 **≤ 54**（baseline 181/59，扣除 Task 2/3 预测转绿共 5 个 → 54；低于 54 是其它 visitor 类 bonus，高于 54 即回归）。17 个无关红（`DruidNativeParserTest` 15 + `CreateLineageParserImplTest` 2）维持原样，不在本次范围。
